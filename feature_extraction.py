...
def extract_nbow(pmids, gazetteer_file, count_file, text_dir):
    ...
    # Read in features
    with open(gazetteer_file, "rb") as fo:
        tfidf = pickle.load(fo)
    
    unicode_gaz = tfidf.vocabulary_.keys()
    gazetteer = [str(w) for w in unicode_gaz]
    ...
    for i, pmid in enumerate(pmids):
        ...
        with open(file_, "r") as fo:
            text = fo.read()
            text_list[i] = text
    result_array = tfidf.transform(text_list).toarray()
...
def extract_cogat(pmids, gazetteer_file, count_file, text_dir, tfidf_file):
    """
    Creates feature table for Cognitive Atlas terms from full, unstemmed text.
    Just a first pass.
    """
    folder_name = os.path.basename(os.path.normpath(text_dir))
    par_dir = os.path.dirname(os.path.normpath(text_dir))
    out_dir = os.path.join(par_dir, "cogat_"+folder_name)
    tfidf_file = os.path.splitext(gazetteer_file)[0]+".pkl"

    # Read in features
    cogat_df = pd.read_csv(gazetteer_file)
    gazetteer = sorted(cogat_df["id"].unique().tolist())
    with open(tfidf_file, "rb") as fo:
        tfidf = pickle.load(fo)
    ...
    for i, pmid in enumerate(pmids):
        ...
        with open(os.path.join(out_dir, pmid+".txt"), "w") as fo:
            fo.write(text)
    
    # Create and save output
    count_array = tfidf.transform(count_array).todense()
...
def extract_titlewords(pmids, gazetteer_file, count_file):
    ...
    for i, pmid in enumerate(pmids):
        ...
        titlewords = [stemmer.stem(word.lower()) for word in titlewords if word.lower() not in stop]
...
def extract_features(data_dir="/home/data/nbc/athena/athena-data/"):
    ...
    """    
    metadata_features = {"titlewords": extract_titlewords,
                         }
    ...
    proctext_dir = os.path.join(data_dir, "text/processed_combined/")
    stemtext_dir = os.path.join(data_dir, "text/stemmed_combined/")
#    reftext_dir = os.path.join(data_dir, "text/full/")
...
#            tfidf_file = os.path.join(gazetteers_dir, feature+".pkl")
#            count_file = os.path.join(feature_dir, "{0}_{1}.csv".format(dataset, feature))
#            fulltext_features[feature](pmids, gazetteer_file, count_file, proctext_dir, tfidf_file)
...
#            print("Completed {0} {1}".format(dataset, feature))
        ...
        # nbow and references
        gazetteer_file = os.path.join(gazetteers_dir, "nbow.pkl")
